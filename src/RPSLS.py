...
def assess_game(user_action, computer_action):
    ...
    if user_action == computer_action:
        ...
        print(f"You picked {user_action.name} and computer picked {computer_action.name} so you lost!")
        result = GameComputerResult.Win
    else:
        print(f"You picked {user_action.name} and computer picked {computer_action.name} so you win!")
...
def get_computer_action():
    ...
    if result == GameComputerResult.Win:
        choice = [option for option in Victories if option not in [last_computer_action,last_user_action]]
        computer_selection = random.choice(Victories[choice])
    elif result == GameComputerResult.Lose:
        computer_selection =  random.choice(Victories[last_user_action])
    else:
